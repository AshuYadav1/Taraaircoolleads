...
export default function ContactForm() {
  ...
  const calculateLeadQuality = () => {
    ...
    // Urgency scoring
    if (watchedUrgency === "today") {
      score += 3;
      reasons.push("Emergency urgency");
    } else if (watchedUrgency === "this-week") {
      score += 2;
      reasons.push("Urgent need");
    }

    // Area scoring
    if (watchedArea && ["Dahisar", "Borivali", "Mira Road"].includes(watchedArea)) {
      score += 2;
      reasons.push("Primary service area");
    }

    // Budget scoring
    if (watchedBudget && ["1000-2000", "2000-5000", "5000-10000", "10000+"].includes(watchedBudget)) {
      score += 2;
      reasons.push("Good budget range");
    }

    // Service scoring
    if (watchedService && ["repair", "installation", "gas-filling"].includes(watchedService)) {
      score += 1;
      reasons.push("High-demand service");
    }
  ...
  const handleLeadQualified = (leadData: any) => {
    // Pre-fill the contact form with qualified lead data
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <p className="text-xl text-gray-600 mb-6">
              We've received your request and will contact you within 5 minutes.
            </p>
            
            {leadQuality && (
              <div className="bg-blue-50 p-4 rounded-lg mb-6">
                <p className="text-blue-800 font-semibold">Lead Quality: {leadQuality}</p>
              </div>
            )}
            ...
            <div className="grid md:grid-cols-2 gap-4 mb-6">
              <a
                ...
                onClick={() => trackEvent("success_page_call")}
                className="bg-trust-blue text-white py-3 px-6 rounded-lg font-semibold hover:bg-trust-blue-light transition-colors"
              >
                Call +91 97025 25317
              </a>
              <a
                href="https://wa.me/919702525317?text=Hi%20I%20just%20submitted%20a%20form%20for%20AC%20service.%20Please%20call%20me%20back."
                onClick={() => trackEvent("success_page_whatsapp")}
                className="bg-green-500 text-white py-3 px-6 rounded-lg font-semibold hover:bg-green-600 transition-colors"
              >
                WhatsApp Now
              </a>
            </div>
            ...
            <div className="text-sm text-gray-500">
              ...
              <p className="mt-2">For immediate assistance, please call us directly.</p>
            </div>
          </div>
        </div>
      </section>
    ...
    <section className="py-16 bg-gray-50" id="contact-form">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-gray-900 mb-4">
            Get Free AC Service Quote
          </h2>
          <p className="text-lg text-gray-600">
            Fill the form below and get instant quote for AC repair, installation, or maintenance
          </p>
        </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl shadow-lg p-8">
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  {errors.acType && (
                    <p className="text-red-500 text-sm mt-1">{errors.acType.message}</p>
                  )}
                </div>
              ...
              <div className="mt-6">
                <button
                  ...
                  className="w-full bg-trust-blue text-white py-3 px-6 rounded-lg font-semibold hover:bg-trust-blue-light transition-colors disabled:opacity-50"
                >
        ...
        <div className="mt-12 bg-white rounded-xl shadow-lg p-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="text-center">
              ...
              <p className="text-gray-600 text-sm">Emergency AC repair available within 2-4 hours</p>
            </div>
            <div className="text-center">
              <div className="text-3xl mb-3">🔧</div>
              <h4 className="font-semibold text-gray-900 mb-2">Licensed Technicians</h4>
              <p className="text-gray-600 text-sm">Certified professionals with 10+ years experience</p>
            </div>
            <div className="text-center">
              <div className="text-3xl mb-3">💰</div>
              <h4 className="font-semibold text-gray-900 mb-2">Transparent Pricing</h4>
              <p className="text-gray-600 text-sm">No hidden charges, upfront pricing with warranty</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {showLeadQualification && (
        <LeadQualification
          ...
          onClose={() => setShowLeadQualification(false)}
        />
      )}
    </section>
  );
